...
public class roadEnv extends Environment
{
    ...
    private Logger logger = Logger.getLogger(roadEnv.class.getName());

    //random numbers with fixed seed
		...
		if(RoadScenarioVar.m_Pause)
			{
					...
					if(ag.equals("vehicle"))
					{
						...
						if(RoadScenarioVar.g_vehicleMeter+12<=2010)
							RoadScenarioVar.g_vehicleMeter+=9;		//vehicle speed originally 12
						else
						{
							RoadScenarioVar.g_vehicleMeter=0;
							RoadScenarioVar.vehicle_i++;
...
private void HandleStop_v(String ag, Structure action)
	...
	{	
		//use the repeat number as the seed
		//check this https://stackoverflow.com/questions/27760450/first-random-number-after-setseed-in-java-always-similar
		int loopNo = RoadScenarioVar.total_Score_number * 1;
		double random = num.random(loopNo);

		// logger.info(String.format("generated random = %f", random));

		RoadScenarioVar.startTime=System.currentTimeMillis(); 
		int px,py;
		// px = (int)(Math.random(0)*1850)+50;
		// py = (int)(Math.random(0)*450)+10;
		px = (int)(random*1850)+50;
		py = (int)(random*450)+10;
		if(py>=80&&py<=220)
			py=80;
		if(py>220&&py<=345)
			py=345;
		if(px<=200||px>=1600)
			px=200+(int)(random*100);//px=200+(int)(Math.random()*100);
		
		if(py<=500&&RoadScenarioVar.move_mode_ID==3)
			{
			py=20+(int)(random*40);//py=20+(int)(Math.random()*40);
			}
